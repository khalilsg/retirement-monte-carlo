...
export function renderFan(r) {
  ...
  for (let i = 0; i <= 5; i++) { const v = (ymax / 5) * i, yy = yOf(v); g.appendChild(svgEl("line", { x1: m.l, y1: yy, x2: W - m.r, y2: yy })); const t = svgEl("text", { x: m.l - 8 * k, y: yy + 3.5 * k, "text-anchor": "end" }); t.textContent = fmtMoney(v); ax.appendChild(t); }
  // Fewer ticks when the labels are large, or they overlap each other.
  const xstep = h <= 35 ? (k > 1.5 ? 10 : 5) : (k > 1.5 ? 20 : 10);
  for (let y = 0; y <= h; y += xstep) { const t = svgEl("text", { x: xOf(y), y: H - m.b + 16 * k, "text-anchor": "middle" }); t.textContent = r.ca + y; ax.appendChild(t); }
  ...
  if (r.A > 0) { const rx = xOf(r.A); svg.appendChild(svgEl("line", { x1: rx, x2: rx, y1: m.t, y2: m.t + ih, stroke: "var(--ink)", "stroke-width": 1.2, "stroke-dasharray": "4 3", opacity: .5 })); const rl = svgEl("text", { x: rx + 4, y: m.t + 10 * k, class: "axis-title" }); rl.setAttribute("font-weight", "600"); rl.textContent = "retires at " + r.retAge; svg.appendChild(rl); }
  const xt = svgEl("text", { class: "axis-title", x: m.l + iw / 2, y: H - 3 * k, "text-anchor": "middle" }); xt.textContent = "Age"; svg.appendChild(xt);
  const at = svgEl("text", { class: "axis-title", transform: `translate(${14 * k},${m.t + ih / 2}) rotate(-90)`, "text-anchor": "middle" }); at.textContent = isPrivate() ? "Balance (× today's)" : "Balance (today's $)"; svg.appendChild(at);
